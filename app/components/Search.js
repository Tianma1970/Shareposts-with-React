...
function Search() {
  ...
  useEffect(() => {
    ...
    //we need to clean up after us when the search screen is closed
  ...
  //function to be run at any time you press a key on your keyboard
  function SearchKeyPressHandler(e) {
    //every key has its own keyvode 27 is "esc"
    if (e.keyCode == 27) {
      ...
      <div className="search-overlay-bottom">
        <div className="container container--narrow py-3">
          ...
          <div className={"live-search-results " + (state.show == "results" ? "live-search-results--visible" : "")}>
            ...
            {!Boolean(state.results.length) && <p className="alert alert-danger text-center shadow-sm">Sorry, we could nor find any results for that search.</p>}
          </div>
        </div>
      </div>
    </div>
